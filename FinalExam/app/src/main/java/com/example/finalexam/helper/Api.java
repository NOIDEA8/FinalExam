...
public interface Api {
    ...
    Call<InfoProjectList> getSelfProjects(@Header ("token") String token,@Query("userId") int useId);


    @GET("user/showHaveMonitorPermissionProjects")
    Call<InfoProjectList> getHaveMonitorProjects(@Header ("token") String token,@Query("userId") int useId);

    //获取我申请过监控的项目

    @GET("user/myApplicationOnMonitorProject")
    Call<InfoProjectList> getApplyingMonitorProject(@Header ("token") String token,@Query("userId") int useId);

    //获取我发布或更改的项目
    @GET("user/myApplicationProject")
    Call<InfoProjectList> getMyApplicationProject(@Header ("token") String token,@Query("userId") int useId);
    //收到的申请
    ...
    Call<InfoShowAllProject> getFrezonOrNotProject(@Header ("token") String token,@Query("projectStatus") int projectStatus);//0冻结，1未冻结

    //获取不同审核状态的项目
    @GET("admin/pagedQueryProjectApplication")
    Call<InfoShowAllProject> getReviewOrNotProject(@Header ("token") String token,@Query("applicationStatus") int applicationStatus);//0待审核，2被拒绝
